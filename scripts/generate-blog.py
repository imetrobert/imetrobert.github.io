...
# Google Analytics tag constant - ADDED FOR AUTOMATIC INCLUSION
...
def create_html_blog_post(content, title, excerpt):
    """Create complete HTML blog post with Google Analytics and PROPERLY FORMATTED content sections"""
    current_date = datetime.now()
...
def create_blog_index_html(posts):
    """Create blog index page with Google Analytics"""
    if not posts:
        return None
