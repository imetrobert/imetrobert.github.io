...
- "remote_montreal" — genuinely fully remote, AND a candidate based in Montreal/Quebec/Canada is eligible. The employer's own office location is irrelevant — a US, European, or fully distributed company is fine as long as a Canadian remote employee is actually permitted. Do not require the posting to name Canada explicitly: a posting that is simply silent on geography, with no restrictive signal either way, still counts here. What disqualifies a posting from this tier is a restriction to somewhere else, whether stated outright or only implied. Watch for US-only tells even when the posting never says "US only": "must be authorized to work in the United States without sponsorship", a requirement to reside in one or more named US states, pay stated only as a US salary band with US-specific benefits (401(k), US federal holidays), EEO/OFCCP/E-Verify boilerplate (US-specific legal language), or references to needing a US Social Security Number or I-9. Any one of those, without an explicit statement welcoming Canadian or international candidates, means this is NOT "remote_montreal" — classify it "not_montreal" instead (unless the posting separately offers a real Montreal-area office option, in which case judge that option on its own under onsite_close/onsite_far).
